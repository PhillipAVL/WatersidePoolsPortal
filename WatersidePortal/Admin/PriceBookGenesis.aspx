<%@ Page MaintainScrollPositionOnPostback="true" Title="Pricebook Genesis" Language="C#" MasterPageFile="~/Shared/MasterPages/SiteLayoutAdmin.Master" AutoEventWireup="true" CodeBehind="PriceBookGenesis.aspx.cs" Inherits="WatersidePortal.Admin.PriceBookGenesis" %>
                ...
                <asp:GridView ID="GridView_Items" runat="server" CssClass ="table table-striped table-bordered table-hover" AllowSorting="True" AutoGenerateColumns="False" DataKeyNames="ItemID" DataSourceID="Items" AutoGenerateEditButton="True" OnRowEditing="GridView_Items_RowEditing">
                    <Columns>
                        ...
                        <asp:BoundField DataField="ImageString" HeaderText="ImageString" SortExpression="ImageString" ControlStyle-CssClass="form-control"/>
                            ...
                            <br />
                            <asp:SqlDataSource ID="Items" runat="server" ConnectionString="<%$ ConnectionStrings:WatersidePortal_dbConnectionString %>" SelectCommand="SELECT * FROM [PriceBookGenesis]" DeleteCommand="DELETE FROM [PriceBookGenesis] WHERE [ItemID] = @ItemID" InsertCommand="INSERT INTO [PriceBookGenesis] ([Category], [Subcategory], [Item], [Description], [Unit], [CustomerPrice], [COGS], [ImageString]) VALUES (@Category, @Subcategory, @Item, @Description, @Unit, @CustomerPrice, @COGS, @ImageString)" UpdateCommand="UPDATE [PriceBookGenesis] SET [Category] = @Category, [Subcategory] = @Subcategory, [Item] = @Item, [Description] = @Description, [Unit] = @Unit, [CustomerPrice] = @CustomerPrice, [COGS] = @COGS, [ImageString] = @ImageString WHERE [ItemID] = @ItemID" >
<DeleteParameters>

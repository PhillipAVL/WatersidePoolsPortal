<%@ Page MaintainScrollPositionOnPostback="true" Title="Pricebook" Language="C#" MasterPageFile="~/Shared/MasterPages/SiteLayoutAdmin.Master" AutoEventWireup="true" CodeBehind="CSubsubPriceBook.aspx.cs" Inherits="WatersidePortal.CSubsubPriceBook" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="ContentBody" runat="server">
                                    ...
                                    <div class="table-responsive">
                                        <asp:GridView ID="GridView_Items" runat="server" CssClass="table table-striped table-bordered table-hover" AllowSorting="True" AutoGenerateColumns="False" DataSourceID="SqlDataSource1" OnRowEditing="GridView_Items_RowEditing" OnSelectedIndexChanged="Selected">
                                            <Columns>
                                                ...
                                                <asp:BoundField ControlStyle-CssClass="form-control" DataField="Subsubcategory" HeaderText="Subsubcategory" ItemStyle-Width="97%">
                                                    <ControlStyle CssClass="form-control" />
                                                </asp:BoundField>
                                            </Columns>
                                        </asp:GridView>
                                        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:WatersidePortal_dbConnectionString %>" SelectCommand="SELECT distinct [Subsubcategory] FROM [PriceBook] WHERE [dbo].[PriceBook].[Category] = 'DECKING' AND [dbo].[PriceBook].[Subcategory] = 'TRAVERTINE' AND Datalength([Subsubcategory]) > 0"></asp:SqlDataSource>
                                    </div>
                                    <div class="table-responsive">
                                        ...
                                        <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:WatersidePortal_dbConnectionString %>" SelectCommand="SELECT [Item], [Description], [CustomerPrice], [Unit], [ItemID] FROM [PriceBook] WHERE [dbo].[PriceBook].[Category] = 'DECKING' AND [dbo].[PriceBook].[Subcategory] = 'Travertine' AND ([dbo].[PriceBook].[Subsubcategory] is null or [dbo].[PriceBook].[Subsubcategory] = '')"></asp:SqlDataSource>
                                    </div>
                                    <br />
                                    <br />
    ...
    <script type="text/javascript">
        if (window.location.href.split('?').length > 1) {
            ...
            window.location.href = "CSubsubPriceBook?DECKING&Travertine";
        }
        function tab(which) {
            if (window.location.href.split('?').length > 1) {
                window.location.href = "CSubsubPriceBook?" + which;
            }
        }
    </script>
</asp:Content>

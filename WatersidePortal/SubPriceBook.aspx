<%@ Page MaintainScrollPositionOnPostback="true" Title="Pricebook" Language="C#" MasterPageFile="~/Shared/MasterPages/SiteLayoutAdmin.Master" AutoEventWireup="true" CodeBehind="SubPriceBook.aspx.cs" Inherits="WatersidePortal.SubPriceBook" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="ContentBody" runat="server">
                        ...
                        <div class="tab-pane fade in active" id="EditDelete">
                            ...
                            <asp:Panel ID="Panel_Items" runat="server" CssClass="panel panel-default">
                                ...
                                <div class="panel-body panel panel-default">
                                    <h4>
                                        Select the subcategory to view more products. These will not be transferred to any shopping carts. (View Only)
                                    </h4>
                                        ...
                                        <div class="table-responsive">
                                            <asp:GridView ID="GridView_Items" runat="server" CssClass ="table table-striped table-bordered table-hover" AllowSorting="True" AutoGenerateColumns="False" DataSourceID="SqlDataSource1" OnRowEditing="GridView_Items_RowEditing" OnSelectedIndexChanged="Selected">
                                                <Columns>
                                                    ...
                                                    <asp:BoundField ControlStyle-CssClass="form-control" DataField="Subcategory" HeaderText="Subcategory" SortExpression="Subcategory" ItemStyle-Width="45%">
                                            ...
                                            <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:WatersidePortal_dbConnectionString %>" SelectCommand="SELECT [Subcategory], STRING_AGG(nullif([Subsubcategory],''), ', ') WITHIN GROUP (ORDER BY [Subsubcategory]) AS Subsub FROM (select distinct [Subsubcategory], [Subcategory] FROM [PriceBook] WHERE [dbo].[PriceBook].[Category] = 'DECKING') x GROUP BY [Subcategory]"></asp:SqlDataSource>
                                        </div>
                                        <div class="table-responsive">
                                            ...
                                            <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:WatersidePortal_dbConnectionString %>" SelectCommand="SELECT [Item], [Description], [CustomerPrice], [Unit] FROM [PriceBook] WHERE [dbo].[PriceBook].[Category] = 'DECKING'"></asp:SqlDataSource>
                                        </div>
    ...
    <script type="text/javascript">
        if (window.location.href.split('?').length > 1) {
            ...
            window.location.href = "SubPriceBook?DECKING";
        }
        function tab(which) {
            if (window.location.href.split('?').length > 1) {
                window.location.href = "SubPriceBook?" + which;
            }
        }
    </script>
</asp:Content>
